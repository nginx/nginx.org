...
Изменения в nginx 1.31.6                                          15.09.2026

    *) Безопасность: при использовании HTTP/3 с OpenSSL 3.5.0 и более
       ранними версиями в определённых конфигурациях могло произойти
       переполнение буфера в рабочем процессе (CVE-2026-90439).
       Спасибо Banny Liao.

    *) Изменение: теперь расширение QUIC transport parameters, полученное в
       SSL-соединении, всегда игнорируется.

    *) Исправление: процедура обновления исполняемого файла не работала,
       если был указан сокет control API и новый исполняемый файл nginx был
       собран с модулем ngx_http_perl_module.

    *) Исправление: ошибка при вычислении предиката в предикатном location'е
       игнорировалась, и такой предикат считался ложным.

    *) Исправление: ошибка при поиске вложенного location'а могла
       игнорироваться, если на текущем уровне были настроены location'ы,
       заданные регулярными выражениями или предикатами.

    *) Исправление: при чтении конфигурации мог произойти segmentation
       fault, если использовалась директива geo с параметром ranges и
       соответствующий файл бинарной базы был повреждён.


Изменения в nginx 1.31.5                                          02.09.2026

    *) Добавление: control API.

    *) Добавление: предикатные location'ы.

    *) Добавление: модуль ngx_http_json_module.

    *) Добавление: директива client_body_early_read.

    *) Исправление: в рабочем процессе могло произойти обращение к ранее
       освобождённой памяти, если использовалось проксирование с
       буферизацией и при отправке ответа HTTP/2-клиенту произошла ошибка.

    *) Исправление: рабочий процесс мог не завершиться или в логах могли
       появляться сообщения "accept4() failed (9: Bad file descriptor)",
       если у рабочего процесса закончились файловые дескрипторы перед его
       плавным завершением.

    *) Исправление: запросы к FastCGI- и uwsgi-бэкендам были некорректными,
       если имя параметра было слишком длинным.

    *) Исправления в HTTP/3, ngx_http_slice_module и
       ngx_http_memcached_module.


Изменения в nginx 1.31.4                                          19.08.2026

    *) Добавление: директива proxy_protocol в модулях stream и mail теперь
       поддерживает протокол PROXY версии 2.

    *) Изменение: теперь запросы к HTTP/2 и gRPC-бэкендам всегда
       отправляются с псевдо-заголовком ":authority", а запросы к
       HTTP/1.1-бэкендам - с заголовком "Host".

    *) Исправление: в рабочем процессе мог произойти segmentation fault,
       если использовался метод select.

    *) Исправление: неполные gRPC-ответы с ненулевым заголовком
       "Content-Length" теперь считаются некорректными.

    *) Исправление: в бинарной совместимости со сторонними модулями,
       использующими script codes; ошибка появилась в 1.31.3.

    *) Исправление: в модуле ngx_http_perl_module.
       Спасибо David Carlier.

    *) Исправления в HTTP/2, HTTP/3, ngx_http_image_filter_module и
       ngx_http_grpc_module.


Изменения в nginx 1.31.3                                          15.07.2026

    *) Безопасность: при использовании директивы map c регулярными
       выражениями могло произойти переполнение буфера в рабочем процессе,
       если map-переменная была указана в строковом выражении после
       выделения, модифицируемого этим блоком map; похожая проблема могла
       произойти при использовании некешируемой переменной в строковом
       выражении (CVE-2026-42533).
       Спасибо Mufeed VH из Winfunc Research и Максиму Дунину.

    *) Безопасность: при использовании неименованных выделений с директивой
       slice или фоновым обновлением кэша мог произойти доступ к
       неинициализированной памяти, что могло привести к отправке клиенту
       содержимого памяти рабочего процесса или вызвать segmentation fault в
       рабочем процессе (CVE-2026-60005).

    *) Безопасность: при использовании ngx_http_ssi_filter_module специально
       созданный ответ от проксируемого бэкенда мог привести к обращению к
       ранее освобождённой памяти (CVE-2026-56434).
       Спасибо P4P3R-HAK.

    *) Изменение: размер заголовков и трейлеров HTTP/2-ответа в модуле
       ngx_http_proxy_v2_module и ngx_http_grpc_module теперь ограничены
       директивой proxy_buffer_size и grpc_buffer_size.

    *) Изменение: загрузка внешних сущностей теперь отключена в модуле
       ngx_http_xslt_filter_module.
       Спасибо Максиму Дунину.

    *) Добавление: директива xml_external_entities в модуле
       ngx_http_xslt_filter_module.
       Спасибо Максиму Дунину.

    *) Добавление: директивы proxy_socket_sndbuf, proxy_socket_rcvbuf,
       fastcgi_socket_sndbuf, fastcgi_socket_rcvbuf, grpc_socket_sndbuf,
       grpc_socket_rcvbuf, scgi_socket_sndbuf, scgi_socket_rcvbuf,
       uwsgi_socket_sndbuf, uwsgi_socket_rcvbuf, tunnel_socket_sndbuf и
       tunnel_socket_rcvbuf.

    *) Добавление: определение размера строки кэша процессора для
       loongarch64.
       Спасибо Miao Wang.

    *) Исправление: теперь nginx возвращает ошибку, если в запросе по
       протоколу HTTP/2 псевдо-заголовки следуют за обычными заголовками.

    *) Исправление: в управлении потоком в модуле ngx_http_v2_module.

    *) Исправление: при отправке закэшированного ответа HTTP/2 в модуле
       ngx_http_proxy_v2_module в логах могли появляться сообщения "[error]
       upstream sent frame for unknown stream" и "[crit] cache file ...
       contains invalid header", если использовалась директива
       proxy_cache_revalidate.

    *) Исправление: nginx мог передавать заголовок "Upgrade" клиенту в
       ответе HTTP/2 или HTTP/3.

    *) Исправление: в модуле ngx_http_perl_module.
       Спасибо Максиму Дунину.

    *) Исправление: запрет фрагментации IPv6 мог не работать при
       использовании QUIC на некоторых системах.

    *) Исправление: в модуле ngx_http_auth_basic_module на Solaris.

    *) Исправления и улучшения в модуле ngx_http_tunnel_module.


Изменения в nginx 1.31.2                                          17.06.2026

    *) Безопасность: при использовании HTTP/3 обработка специально созданной
       QUIC-сессии могла приводить к обращению к ранее освобождённой памяти,
       что позволяло атакующему повредить память рабочего процесса или
       вызвать segmentation fault в рабочем процессе (CVE-2026-42530).
       Спасибо Trung Nguyen из CyStack.

    *) Безопасность: при проксировании специально созданного запроса на
       HTTP/2 или gRPC-бэкенд с использованием директив
       "ignore_invalid_headers off;" и "large_client_header_buffers" с
       большим значением могло происходить переполнение буфера в рабочем
       процессе, что позволяло атакующему повредить память рабочего процесса
       или вызвать segmentation fault в рабочем процессе (CVE-2026-42055).
    ...
    *) Изменение: теперь переменная $request_id использует SipHash-2-4.

    *) Добавление: переменная $ssl_sigalgs.

    *) Исправление: переменная заданная директивой split_clients могла быть
       пустой, если все проценты были заданы явно и их сумма была равна
       100%.

    *) Исправление: сравнение хешей secure_link за константное время.
       Спасибо kodareef5.
